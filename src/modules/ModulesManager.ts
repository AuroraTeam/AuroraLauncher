...
export class ModulesManager {
    private modulesPath = path.resolve(__dirname, `modules`)
    ...
    async loadModules() {
        try{
            ...
            if(!fs.existsSync(path.resolve(this.modulesPath, "plugins.json")) || folders.length === 0) return LogHelper.info("Modules dir empty. Skip loading")

            const moduleList = JSON.parse(fs.readFileSync(path.resolve(this.modulesPath, "plugins.json")).toString())
            const startTime = Date.now()

            await Promise.all(moduleList.map((plugin: string) => {
    ...
    private static async loadModule(moduleName: string) {
        try{
            const modulesPath = path.resolve(__dirname, `modules/${moduleName}`)
            const packageJson = JSON.parse(fs.readFileSync(path.resolve(__dirname, `modules/${moduleName}/package.json`)).toString())

            /* Проверяем, все ли депенденсы установлены */
            if(packageJson.dependencies && !await fsProm.access(`${path.resolve(modulesPath, `node_modules`)}`).then(() => true).catch(() => false)) {
            ...
            if(packageJson.scripts && packageJson.scripts.build && !await fsProm.access(`${path.resolve(modulesPath, `lib`)}`).then(() => true).catch(() => false)) {
    ...
    private static classСheck(object: any){
        ...
        const isPrototypeConstructorClass = object.prototype.constructor
    ...
    private async runTasks(list: any[], task: string) {
        return Promise.all(list.filter((module: any) => module.instance[task]).map((el: any) => this.issueTask(el, task)));
    }
}
