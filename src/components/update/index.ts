...
export class UpdateManager {
    ...
    private readonly fileType: string = SystemHelper.isStandalone() ? this.fileTypeMap[SystemHelper.getPlatform()] : "js";
    private readonly execFileName: string = SystemHelper.isStandalone() ? `LauncherServer-${SystemHelper.getPlatform()}` : "LauncherServer.js";
    ...
    public async checkUpdate(): Promise<Version | void> {
        ...
        const versionsData = await this.getVersionsData().catch(() => undefined);
        ...
        const latestVersionData = versionsData.versions.find((versionData: Version) => versionData.version === latestVersion);
