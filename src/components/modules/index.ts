...
public static modulesList = new Map<ModuleInfo, LauncherServerModule[]>();
    ...
    async loadModules(): Promise<void> {
        try {
            ...
            const files = await fs.readdir(StorageHelper.modulesDir, { withFileTypes: true });
            const moduleFiles = files.filter((file) => file.isFile() && file.name.endsWith(".js"));
            ...
            await Promise.all(moduleFiles.map((file) => this.loadModule(file.name)));
    ...
    private async loadModule(moduleName: string): Promise<void> {
        try {
            ...
            LogHelper.debug(error.message);
            LogHelper.error(this.langManager.getTranslate.ModulesManager.moduleLoadingErr, moduleName);
        }
    }
