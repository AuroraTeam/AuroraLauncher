...
export class NeoForgeManager extends MojangManager {
    ...
    async downloadClient(gameVersion: string, clientName: string) {
        if (await this.downloadForge(gameVersion)) {
            ...
            LogHelper.error(
                this.langManager.getTranslate.DownloadManager.NeoforgeManager.info.errForgeVersion,
            );
    }
    ...
    libParser(libraries: Array<Libraries>): Array<ProfileLibrary> {
        ...
        const minecraftDir = readdirSync(
            resolve(this.#tempDir, "libraries/net/minecraft/client"),
        )[0];
        const files = readdirSync(
            resolve(this.#tempDir, "libraries/net/minecraft/client", minecraftDir),
